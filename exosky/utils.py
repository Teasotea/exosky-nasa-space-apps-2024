...
def get_exoplanet_data():
    exoplanets = NasaExoplanetArchive.query_criteria(
        ...
        select="pl_name, sy_dist, ra, dec, st_mass, st_rad",
...
def filter_exoplanets_within_distance(df_exoplanets, min_distance=0, max_distance=10):
    if isinstance(min_distance, (tuple, list)) or isinstance(
        max_distance, (tuple, list)
    ):
        raise ValueError("min_distance and max_distance must be single scalar values")

    return df_exoplanets[
        (df_exoplanets["sy_dist"] >= min_distance)
        & (df_exoplanets["sy_dist"] <= max_distance)
    ]
...
def plot_smth(ra_arr, dec_arr, mag_arr, bp_rp_arr):
    ...
    sc = ax.scatter(
        ra_rad, dec_rad, s=size, c=bp_rp_arr, cmap=cmap, norm=norm, alpha=0.75
    )
